...
budget_data <- read_csv("data/analysis_data/budget.csv")

# Read in the demographic data
demographic_data <- read_csv("data/analysis_data/demographic.csv")
...
# Ensure that year columns and ward number are numeric
budget_data <- budget_data |>
  mutate(across(all_of(c(year_cols, "Total 10 Year", "Ward Number")), as.numeric))
...
merged_table %>%
  arrange(desc(total_funding)) %>%
  kable(
    col.names = c("Ward Number", "Total Funding", "Total Population", "Median Income", "Low Income Prevalence", "Tenant Households Spending ≥30% on Shelter"),
    format = "html",
